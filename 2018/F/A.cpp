...
int check(string sub, int len, vector<string> subs[])
{
    for (auto i : subs[len])
        if (sub == i)
            return true;
...
void solve()
{
    ...
    vector<string> subs[l*l];

    for (int len = 1; len <= l; len++)
    {
        for (int start = 0; start <= l - len; start++)
        {
            string subb = b.substr(start, len);
    ...
    for (int len = 1; len <= l; len++)
    {
        for (int start = 0; start <= l - len; start++)
        {
            string suba = a.substr(start, len);
            sort(suba.begin(), suba.end());
            if (check(suba, len, subs))
                count++;
        }
    }
