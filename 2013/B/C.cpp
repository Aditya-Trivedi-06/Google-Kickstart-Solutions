...
int minsteps(int row, int col)
{
    // cout << x << ' ' << y << endl;

    if (row == gy and col == gx)
        return 1;
    ...
    return 1 + min4(minsteps(row + 1, col), minsteps(row - 1, col), minsteps(row, col + 1), minsteps(row, col - 1));
}
